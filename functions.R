...
get_acc_benefits <- function(a_sgr,bf) {
  ...
  benefit_factor <- bf
  acc_benefits <-
    c(0,sapply((acc_sal[2:6] - acc_sal[1]),function(b)
      b * benefit_factor),
      sapply(((
        acc_sal[7:length(acc_sal)] - acc_sal[2:(length(acc_sal) - 5)]
      ) * yos_xy[7:length(yos_xy)]),
      function(c)
        (c * benefit_factor) / 5))
...
get_acc_benefit_r <- function(a_sgr,sgr,bf) {
  exp_sal_r <-
    as.numeric(get_exp_sal_r(a_sgr,sgr))[1:(length(age) - 5)]
  ...
  acc_benefit_r <-
    (exp_sal_r + (exp_sal_r * ((1+sgr) ^ (-1))) + (exp_sal_r * ((1+sgr) ^ (-2))) + (exp_sal_r * ((1+sgr) ^ (-3))) + (exp_sal_r * ((1+sgr) ^ (-4)))) / (5) * bf * (length(age) - 1)
  #exp_sal_r*(1+((1+sgr)^-1)+((1+sgr)^-2)+((1+sgr)^-3)+((1+sgr)^-4))*bf*(length(age)-1)/5
  #exp_sal_r*((1+sgr)*(1^(0)+1^(-1)+1^(-2)+1^(-3)+1^(-4)))*bf*(length(age)-1)/5
  ...
  acc_benefit_r3 <-
    c((acc_benefit_r3 / 5) * bf * (length(age) - 1),acc_benefit_r1[5] * bf *
        (length(age) - 1) / 5)
...
get_acc_benefit_65 <- function(a_sgr,sgr,bf) {
  acc_benefit_65 <-
    as.numeric(get_acc_benefit_r(a_sgr,sgr,bf)[length(get_acc_benefit_r(a_sgr,sgr,bf))])
  # accumulated benefit at age r
...
get_tla_n <- function(i,mort) {
  ...
  for (j in 1:n) {
    ...
    if (j < n)
      pxr <-
        cumprod(c(1, px[j:(n - 1)]))
    else
      pxr <-
        1      # survival probability to retirment at age x. Note that participant always survives at the beginning of age x
    SS  <- scale[j:n] / scale[j]                # scale
    tla[j] <-
      sum(SS * v * pxr)                # computing annuity value at j
  }
  return(tla)
}
...
get_rPVFBx <- function(i,a_sgr,sgr,cola,bf,mort) {
  ...
  rPVFBx <-
    get_vrx(i) * get_rxpxT(mort) * get_acc_benefit_r(a_sgr,sgr,bf) * get_ar(i,cola,mort)
...
get_PVTC_t <- function(i,a_sgr,sgr,cola,bf,mort,vesting)
{
  ...
  for (k in 1:(length(age) - 1))
    pvtc_t[k] <-
      c(get_PVTC(i,a_sgr,sgr,cola,bf,mort,vesting,age[k]))
  return(pvtc_t)
}
...
get_AAL <- function(ea,i,a_sgr,sgr,cola,bf,cm,mort,vesting) {
  age_information(ea)
  ...
  if (cm == 'EAN')
    aal <-
    k - (get_NC(ea,i,a_sgr,sgr,cola,bf,cm,mort,vesting) * c(get_tla_t(i,sgr,mort),get_tla(i,sgr,mort,age[length(age)])))
  else
...
############################## NC for cost methods ##############################
get_NC <- function(ea,i,a_sgr,sgr,cola,bf,cm,mort,vesting) {
  age_information(ea)
  rPVFBx <- get_rPVFBx(i,a_sgr,sgr,cola,bf,mort)
...
get_median_asset <-
  function(ea,median_p,median_dr,median_sgr,a_sgr,cola,bf,cm,mort,vesting) {
    i <- median_dr
    sgr <- median_sgr
    median_asset <-
      median_p * get_AAL(ea,i,a_sgr,sgr,cola,bf,cm,mort,vesting)
    return(median_asset)
  }
...
get_FR <-
  function(pop,ea,median_p,median_dr,median_sgr,i,a_sgr,sgr,cola,bf,cm,mort,vesting) {
    age_information(ea)
    fr <-
      sum(pop*get_median_asset(ea,median_p,median_dr,median_sgr,a_sgr,cola,bf,cm,mort,vesting)[2:length(age)]
      ) / sum(pop*get_AAL(ea,i,a_sgr,sgr,cola,bf,cm,mort,vesting)[2:length(age)])
    fr[is.na(fr)] <- 0
...
get_ARC <-
  function(pop,ea,median_p,median_dr,median_sgr,i,a_sgr,sgr,cola,bf,cm,mort,vesting,amortization) {
    age_information(ea)
    arc <-(pop*(get_NC(ea,i,a_sgr,sgr,cola,bf,cm,mort,vesting)) + 
          (((pop*(get_AAL(ea,i,a_sgr,sgr,cola,bf,cm,mort,vesting))) -
           (pop*get_median_asset(ea, median_p,median_dr,median_sgr,a_sgr,cola,bf,cm,mort,vesting)))
